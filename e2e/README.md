...
默认行为是始终通过 `mvn spring-boot:run` 启动隔离的临时后端，并使用空状态文件保证每次运行从种子数据开始；运行结束后会清理该进程。为避免状态污染，默认不会复用已经运行的后端。若确实需要连接外部环境，可显式设置 `E2E_START_BACKEND=0`。
...
测试账号使用每次运行生成的唯一用户名；种子账号仅用于跨角色校验。失败时报告会记录场景、接口、期望/实际状态和响应摘要，便于复现。若后端启动失败，仍会生成三种报告，并在环境字段中记录启动错误和后端日志。
