...
该目录承载 `identity-service`、`merchant-service`、`order-service` 三个独立 Spring Boot 入口。当前可运行的内部业务接口分别为 9、12、18 个：identity 负责账户/资料/地址，merchant 负责商家/商品/团购，order 负责购物车/订单详情/支付/履约/券码/评价。生产配置下三个服务分别连接 `life_assistant_identity`、`life_assistant_merchant`、`life_assistant_order`，backend 仍提供 `/api/v1/**` 兼容入口；三个数据库位于同一 MySQL 实例，但服务表已具有独立逻辑数据库边界。
...
单体网关默认保持 `monolith` 路由。Compose/Kubernetes 可打开三类远程路由；设置对应 `LUMALIFE_*_REMOTE_ENABLED` 和 `*_BACKFILL_COMPLETED` 为 `false` 即可逐服务回滚。通过 `GET /internal/migration/status` 查看实时路由。merchant 的分类、收藏、客服/AI 仍可能回落单体；筛选排序已由远程 adapter 和 `MerchantStore` 实现，团购在 JDBC 可用时持久化到 `group_deal`。order-service 会保存商品名称、商家名称和配送地址快照，订单详情不依赖商家服务在线才能显示历史信息。

远程切流后，管理员看板由 backend 的 `RemoteMetricsServicePort` 聚合三个服务的只读投影：identity 提供安全账号摘要，merchant 提供商家目录，order 提供订单/履约/评价统计；`DemoStore` 仅在单体回退模式提供基线实现。order-service 的状态变更会在本地事务中写入 `service_outbox_event`，可通过 `LUMALIFE_EVENTS_PUBLISHER_ENABLED=true` 和 `LUMALIFE_EVENTS_SINK_URL` 启用 HTTP 发布器；这不是已经部署的消息总线，也不等同于完整 Inbox/Saga。AI 客服仍属于 BFF 兼容能力，库存仍是同步 HTTP 预占/确认/释放，因此不能据此宣称全量业务已经完成微服务化。
