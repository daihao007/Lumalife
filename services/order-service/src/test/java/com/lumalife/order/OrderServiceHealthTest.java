...
class OrderServiceHealthTest {
  ...
  void echoesAValidRequestIdOnSuccessfulProbeResponses() {
    ...
    headers.set("X-Request-Id", "d07-order-001");
    ResponseEntity<String> response = http.exchange("/actuator/health/readiness", HttpMethod.GET,
      new HttpEntity<>(headers), String.class);
    assertThat(response.getHeaders().getFirst("X-Request-Id")).isEqualTo("d07-order-001");
  }
}
