...
/** Publishes the order outbox to a durable topic exchange. Replays are safe because consumers use Inbox keys. */
@Component
@ConditionalOnProperty(name = "lumalife.events.broker.enabled", havingValue = "true")
public class RabbitOrderOutboxPublisher {
  private final JdbcTemplate jdbc;
  private final RabbitTemplate rabbit;
  private final ObjectMapper mapper;
  private final String exchange;

  public RabbitOrderOutboxPublisher(ObjectProvider<JdbcTemplate> jdbcProvider, RabbitTemplate rabbit,
  ...
  public void publishPending() {
    ...
    List<OutboxEvent> events = jdbc.query(
        "SELECT id,aggregate_type,aggregate_id,event_type,payload,occurred_at FROM service_outbox_event "
            + "WHERE status IN ('PENDING','FAILED') ORDER BY id LIMIT 50",
        (rs, row) -> new OutboxEvent(rs.getLong("id"), rs.getString("aggregate_type"),
            rs.getLong("aggregate_id"), rs.getString("event_type"), rs.getString("payload"), rs.getTimestamp("occurred_at")));
  ...
  private void publish(OutboxEvent event) {
    try {
      ...
      String message = mapper.writeValueAsString(new EventEnvelope("order-outbox-" + event.id(), event.aggregateType(),
          event.aggregateId(), event.eventType(), payload, event.occurredAt().toInstant().toString()));
  ...
  record OutboxEvent(long id, String aggregateType, long aggregateId, String eventType, String payload, Timestamp occurredAt) {}
  record EventEnvelope(String eventId, String aggregateType, long aggregateId, String eventType, JsonNode payload, String occurredAt) {}
}
