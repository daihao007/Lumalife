...
- 自动化测试包含后端业务规则测试和 Web 层权限集成测试，`monolith-start` 旧标签为 40 个，当前候选基线实测 59 个（37 个业务规则测试 + 22 个接口集成测试）；详见 [测试报告](docs/07_测试报告.md)、[测试基线与缺口矩阵](docs/15_测试基线与缺口矩阵_2026-08-25.md) 与 [单体基线与范围冻结记录](docs/12_单体基线与范围冻结记录.md)。
