...
本说明书覆盖 `frontend/src` 与 `backend/src/main/java/com/lumalife` 的当前实现，不覆盖真实支付渠道、数据库事务、生产级令牌服务和自动化运维平台。Docker Compose 中预留的 MySQL 与 Redis 仅作为未来基础设施，不作为本版本的实际调用对象。
...
4. `DemoStore` 负责当前版本集合、种子数据、快照恢复和持久化触发；后续数据库迁移时应在其外部引入 Repository，而不是让页面或控制器绕过服务层。
...
`FavoriteService` 调用 `DemoStore.addFavorite/removeFavorite/listFavorites/listFavoriteMerchants`。实际存储是 `Map<Long, Set<Long>> favorites`，键为用户 ID，值为商家 ID 集合。收藏操作必须校验当前用户，重复添加保持幂等，删除不存在的收藏不改变其他商家收藏。
...
| 操作 | 详细规则 |
|---|---|
| 加入 | 商品必须存在、`listed=true`、数量为正；同商品累加数量 |
| 跨商家 | 购物车已有商品且商家不同，当前实现清空旧购物车后加入新商品 |
...
3. 以购物车中的商品和商家为依据创建外卖 `Order`，复制为 `OrderLine`，保存地址快照。
4. 计算 `totalCent`，初始状态设为 `PENDING_PAYMENT`，写入 `statusTimeline`。
5. 清空购物车并返回订单列表。

同一购物车只允许一个商家，因此通常产生一个订单；方法返回 `List<Order>` 是为了兼容当前 API 设计。
...
| 状态文件异常 | JSON 不存在、格式错误、写入失败 | 告警并使用种子/内存状态继续运行 |
...
1. 支付使用 `clientRequestId` 和订单字段防重复处理；没有 Payment 实体，因此不以支付记录表作为幂等依据。
...
6. state file 写入失败不应让无关读请求崩溃，但应记录日志，生产迁移时改用事务存储。
...
不能将上述内存复杂度和演示响应时间承诺为生产并发指标。接入数据库后需要为订单、商家、商品、券码和 `client_request_id` 建立索引，并为库存与支付设置事务和并发控制。
...
当前 `DemoStore` 作为单进程对象使用，支付幂等、库存扣减和状态变更在同一个服务调用链中完成，但没有数据库事务、跨进程锁或多实例一致性。后续迁移时需将以下操作设为事务边界：订单创建、支付与库存扣减、团购券生成、订单状态流转、券码核销和评价与评分更新。
...
| 单元测试 | `DemoStore.addCart` | 同商家累加、跨商家清空重加、数量修改和删除 |
| 单元测试 | `DemoStore.createDeliveryOrder` | 地址快照、购物车清空、订单行和总额 |
