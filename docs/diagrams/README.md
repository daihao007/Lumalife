...
当前生产微服务的逐用例三层图以 [`final/use-cases/README.md`](final/use-cases/README.md) 为准：UC01~UC09 各有 SYS、COMP、OBJ 三个 Mermaid 源文件及对应 SVG，共 27 对。它们按 `prod,remote` 的 BFF、业务服务、数据库和消息边界绘制。
...
27 张历史三层模型及 27 个 SVG 已于 2026-09-03 机械迁移到 `UCxx-{SYS|COMP|OBJ}-SEQxx`，对应图标识统一为 `REQxx / UCxx / SYS-SEQxx / COMP-SEQxx / OBJ-SEQxx`。迁移仅改编号和链接，没有把历史单体内容重写为微服务；历史图仍用于 `monolith-start`/兼容基线。答辩展示当前生产调用链时使用 `final/current-architecture.mmd` 或 `final/use-cases/`。
