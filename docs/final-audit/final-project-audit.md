...
R26 已使用 Gitleaks 8.30.1 对 `3696741` 当前受跟踪快照与全部 233 个 Git 提交扫描，均为 0 finding；Syft 1.51.0 生成 SPDX 2.3 SBOM，共 287 个包条目。Grype 0.118.0 对该源码 SBOM 发现 11 个开放 npm match（0 Critical、6 High、4 Medium、1 Low），均标记有修复版本。仍有 38 个 `NOASSERTION`，仓库无根 LICENSE，Maven 完整传递依赖和容器镜像未扫描，因此不能宣称全部许可证/漏洞已验证。
...
6. 修复源码 SBOM 扫描中的 11 个 npm match，解析 38 个 `NOASSERTION`，补 Maven 完整传递依赖和容器镜像的许可证/漏洞扫描。
