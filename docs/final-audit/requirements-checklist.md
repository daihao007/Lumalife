...
> 课程基准：《软件工程基础实践》课程任务书（2026 夏季学期），本机外部来源 `软件工程基础实践-2026夏.md`。仓库内没有任务书副本；若授权允许，答辩交付前应纳入 `02_docs`。当前审计基线：`main@f282fc9`。
...
| R03 | 9 个 UC 的需求、系统/组件/对象三层模型 | 文档 10 | ⚠️ 基本完成但证据/文档存在问题 | 相关类仍存在 | 27 个 mmd/27 个 SVG；编号已统一 | d03/d04 仍为单体/兼容基线 | 是：补当前微服务逐用例图 |
| R04 | 同表追溯 REQ/UC/三层设计/代码/测试 | 文档 10 | ⚠️ 基本完成但证据/文档存在问题 | 当前服务代码已索引 | 当前结果已更新；UI 3/3 状态已修正 | `traceability-matrix.md`；旧图编号已迁移 | 是：补当前微服务逐用例图 |
...
| R26 | AI 使用说明、开源来源、无 Secret 泄露 | 学术诚信 | ⚠️ 基本完成但证据/文档存在问题 | 当前 `.env.example`、Secret 注入和 AI fallback 边界已校准 | 当前文本 + Git 文本历史高置信模式 0 命中；无专用 scanner/SBOM | `10_AI使用说明.md`、`security/secret-scan-2026-09-03.md` | 是：专用 scanner + license/SBOM |
