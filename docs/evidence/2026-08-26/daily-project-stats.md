...
| ---: | --- | --- | --- | --- | --- |
| 6 | [#21 统一需求与测试编号并完成用例说明第一版](https://github.com/daihao007/Lumalife/issues/21) | daihao007 | Done | `REQ01~REQ09`、`UC01~UC09`、`TC-UCxx-nn` 已统一；9 个用例均补齐参与者、触发、前置、主流程、异常和结果；48 个后端自动化测试与 15 个手工流程已映射；新增注册角色约束和库存失败不部分写入回归；本地 `mvn test` 48/48、`npm test` 11/11、`npm run build` 通过 | [PR #75](https://github.com/daihao007/Lumalife/pull/75)；commit 待更新 |
