...
> 版本：`draft-2026-08-28`；目标/当前核对基线：`97c399eb8eff8b188002ea25049f31c954a97ab0`；D07 当前实现索引见 [`28_D07服务接口数据归属与需求追溯.md`](./28_D07服务接口数据归属与需求追溯.md)
> 范围：用户认证、商家商品、订单三个业务服务；不包含骑手、派单、轨迹、运力和骑手结算

> 历史快照注记（2026-09-02）：本文保留 D04 的三服务目标设计，不作为当前运行数量或状态来源。当前为 identity、merchant、order、assistant 四个独立服务，业务 mapping 共 64 个；库存 Outbox/Inbox/Saga 已落地，V001～V019 迁移和 UC01～UC09 remote E2E 状态以 [`D07`](./28_D07服务接口数据归属与需求追溯.md)、[`架构矩阵`](./MICROSERVICE_FINAL_ARCHITECTURE_MATRIX.md) 和统一 evidence 为准。
...
当前代码已经有四个独立 Spring Boot 进程和 64 个内部业务 mapping：`IdentityApi`、`MerchantApi`、`OrderApi`、`AssistantApi` 分别监听 8081/8082/8083/8084；`ApiControllers` 仍是 `/api/v1` 兼容入口，远程适配器按迁移开关选择服务或单体 fallback。当前生产部署使用 legacy `life_assistant` 兼容库和三个服务库，identity/merchant/order 分别连接自己的数据库；旧库仍用于兼容回滚。库存预占和消息基础设施已通过 merchant/order Outbox、Inbox、RabbitMQ 和 Saga 落地，V019 增加释放结果与过期重试约束。下文用“当前落地”与“目标拆分”区分事实和设计，避免把规划误报为已部署能力。
