...
> 历史快照注记（2026-09-02）：本文保留 D04 的三服务目标设计，不作为当前运行数量或状态来源。当前为 identity、merchant、order、assistant 四个独立服务，业务 mapping 共 65 个；库存 Outbox/Inbox/Saga 已落地，当前事实以 [`project-facts.md`](project-facts.md) 为准。
...
当前代码已经有四个独立 Spring Boot 进程和 65 个内部业务 mapping：`IdentityApi`、`MerchantApi`、`OrderApi`、`AssistantApi` 分别监听 8081/8082/8083/8084；`ApiControllers` 是 `/api/v1` BFF 入口。`prod,remote` 使用三个服务数据库，legacy `life_assistant` 仅用于单体基线、迁移和回滚。库存预占和消息基础设施已通过 merchant/order Outbox、Inbox、RabbitMQ 和 Saga 落地，V019 增加释放结果与过期重试约束。
