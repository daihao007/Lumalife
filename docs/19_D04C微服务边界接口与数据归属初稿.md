...
> 版本：`draft-2026-08-28`；代码核对基线：`origin/main@12928a3`
...
3. 即时判定使用有预算的同步调用，跨域传播使用 Outbox/Inbox 事件；支付与库存采用可重试、可对账的预占 Saga。

当前代码是单体内模块化实现：`ApiControllers` 仍是 `/api/v1` 兼容入口，领域端口已经隔离身份、商家、订单和指标能力，但数据库、进程和消息基础设施尚未按三个服务物理拆开。下文用“当前落地”与“目标拆分”区分事实和设计，避免把规划误报为已部署能力。

详细的逐接口请求/响应模型继续以 [`docs/16_三服务接口数据归属与契约草案.md`](./16_三服务接口数据归属与契约草案.md) 第 4～11 节和 [`docs/contracts/`](./contracts/) 为准；本文件是 Issue #33 的验收入口和决策摘要。
...
| INT-MER-02 | order → merchant：`POST /internal/v1/inventory/reservations` | 1 s | 同 key 最多 2 次 | 409 库存不足；超时先按 orderId 查询，禁止盲目重建 |
| INT-MER-03 | order → merchant：`POST /internal/v1/inventory/reservations/{orderId}:release` | 1 s | 后台指数退避 | 幂等释放；已支付订单不得释放 |
...
|---|---|---|
| identity | `user_account`、`auth_session`、`refresh_token`、`user_address`、本地 `outbox_event` | `userId`、角色、`merchantId`、显示名投影；order 的地址快照不回写 identity |
...
- `payment_record` 唯一键为 `(userId, orderId, clientRequestId)`；重试返回同一业务结果，同 key 不得用于另一个订单。
...
| 库存预占响应超时 | 503/504 或处理中 | 按 `orderId` 查询原 reservation，再决定重试；禁止生成第二个预占 |
...
| 验证项 | 命令 | 结果（基线 `12928a3`） |
|---|---|---|
| 后端编译和测试 | `cd backend && mvn -B -ntp test` | 72 tests passed |
...
| D04-C-08 | 待决事项 | 注册审核方式、token 方案、评价聚合、指标延迟、`EXPIRED` 规则、v1 废弃周期 | 评审后记录在 Issue/PR，并将契约版本升为 `frozen-v1` |
