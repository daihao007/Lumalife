...
> 版本：`draft-2026-08-28`；目标/当前核对基线：`fd068de1254233ad14bdd1a24e39d3b10f3b688d`；D07 当前实现索引见 [`28_D07服务接口数据归属与需求追溯.md`](./28_D07服务接口数据归属与需求追溯.md)
...
当前代码已经有三个独立 Spring Boot 进程和 38 个内部业务接口：`IdentityApi`、`MerchantApi`、`OrderApi` 分别监听 8081/8082/8083；`ApiControllers` 仍是 `/api/v1` 兼容入口，远程适配器按迁移开关选择服务或单体 fallback。但数据库仍为共享 MySQL，identity 仍使用 JSON 状态，merchant 团购、库存预占、订单完整明细和消息基础设施尚未完成。下文用“当前落地”与“目标拆分”区分事实和设计，避免把规划误报为已部署能力。
