...
> 历史单体概要设计：本文未完整反映当前 BFF + 4 业务微服务 + 三服务数据库 + RabbitMQ Saga。当前组件边界以 [`architecture/microservices-inventory.md`](architecture/microservices-inventory.md) 和 [`diagrams/final/current-architecture.mmd`](diagrams/final/current-architecture.mmd) 为准；正式提交前须整体改版并重新导出 PDF。
...
- 前后端组件职责及调用边界；
- 本地文件与生产 MySQL 关系表适配器的边界，以及后续多副本演进。

### 1.2 项目背景

LumaLife 面向本地生活服务场景，为用户提供商家发现、商品浏览、收藏、购物车、外卖订单、团购券、评价和智能助手能力，为商家提供经营内容、订单履约、客服和团购券核销能力，为平台管理员提供运营看板。

当前系统是一个可运行的 Web 演示与工程化原型：前端通过 REST API 调用 Spring Boot 后端，领域规则由 `DemoStore` 管理，持久化通过 `BusinessStateRepository` 隔离。本地可使用 `lumalife.state-file`，Docker Compose/Kubernetes 则以 MySQL 规范化关系表作为持久化数据源；MySQL 是容器环境的实际业务依赖，Redis 仍是后续能力。
...
| 持久化 | 本地 `lumalife.state-file`，或 Compose/Kubernetes 的 MySQL 关系表 |
...
| 状态存储 | 领域规则、文件/MySQL 适配、关系表事务同步 | `DemoStore`、`BusinessStateRepository`、MySQL |
...
系统采用“浏览器前端—REST 控制器—领域服务—持久化端口”的分层结构。前端页面不直接访问存储；控制器通过 `SecurityConfig` 取得认证用户并调用对应服务；领域变更通过 `BusinessStateRepository` 写入本地文件或 MySQL 关系表，智能助手服务可选调用 Agnes。
...
flowchart LR
    ...
    PERSIST[BusinessStateRepository]
    FILE[(本地 JSON 状态)]
    MYSQL[(Compose/K8s MySQL 关系表)]
...
`DemoStore` 使用集合或映射表达运行中的领域聚合：账户、令牌、商家、商品、团购、购物车、订单、评价、收藏、会话、商家资料和地址。每次可变业务操作通过持久化端口保存；MySQL 适配器把聚合拆解到对应规范化表，启动时再由关系行重建聚合。

MySQL 适配器提供关系表、事务回滚和单写者命名锁，但领域聚合仍常驻单进程，因此当前不开放多副本并发写入。后续演进应以 `docs/11_数据库持久化迁移计划.md` 为依据，将聚合同步拆成按请求 Repository，而不是让控制器直接依赖数据库。
...
| 参与者 | 平台管理员 |
| 前端入口 | `Admin` |
| 后端组件 | `SecurityConfig`、`ApiControllers`、`AdminDashboardService`、`DemoStore` |
| 主要接口 | `/admin/metrics` |
| 结果 | 管理员获得订单、商家、收入、用户和指标响应中的演示健康字段概览 |
| 异常 | 非平台管理员访问被拒绝，数据为空时显示零值或空状态；真实 Actuator 健康接入列为后续能力 |
...
sequenceDiagram
    ...
    Dashboard-->>AdminUI: health.status=UP（DemoStore 演示字段）
...
| UC09 | 组件级顺序图 | 6.9 | Admin、SecurityConfig、AdminDashboardService、DemoStore |
