...
| 可用性 | Agnes 不可用时使用本地回退；状态文件无法恢复时记录告警并以种子数据启动 |
...
后端从 `backend` 目录通过 Maven/Spring Boot 启动，默认监听 8080。前端从 `frontend` 目录通过 Vite 启动。后端启动时先初始化 `DemoStore` 的种子数据，再尝试恢复可选状态文件；状态文件不存在或格式异常时使用默认数据继续运行。
...
| 异常 | 商家/商品不存在、商品下架、库存不足或跨商家购物车冲突 |
