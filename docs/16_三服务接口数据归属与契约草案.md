...
> 当前候选基线：`2ba74ea9db052143c0eafa5f2c18be2ac555bf85`（D07 复核代码快照；已含三项独立服务、内部 API、远程适配器和 D06 契约测试矩阵）
...
本文是 `docs/15_单体后端审计与三服务拆分草案.md` 的目标契约细化，不把目标数据库、JWT、消息基础设施或 52 个外部接口误报为当前已全量落地。外部 `/api/v1` 契约保持兼容；当前代码已经有三个独立服务和 39 个内部业务接口，但 BFF 仍保留兼容路由，部分能力仍回落到 `DemoStore`。源码、测试和 [`28_D07服务接口数据归属与需求追溯.md`](./28_D07服务接口数据归属与需求追溯.md) 是当前实现事实，本文件和 `docs/contracts/` 是目标契约候选。

### D07 当前实现摘要

| 口径 | 当前事实 |
|---|---|
| 独立服务 | identity 8081、merchant 8082、order 8083，内部业务接口分别 9/12/18，共 39 个 |
| 对外入口 | backend 继续承接 `/api/v1/**`；前端不直接调用 `/internal/v1/**` |
| 数据库 | merchant/order 使用共享 MySQL `life_assistant` 中的逻辑服务表；identity 使用 JSON 状态文件并由 Kubernetes `identity-data` PVC 提供文件载体，不是独立 Schema |
| 未完成能力 | 库存预占/释放、Outbox/Inbox/事件总线、客服/AI/指标服务化仍未完成；order 多商品明细已具备 `service_order_line`/V008，仍需完整快照一致性验证 |
