...
> 历史候选基线：`97c399eb8eff8b188002ea25049f31c954a97ab0`（D07 复核代码快照；当时已含三项独立服务、内部 API、远程适配器和 D06 契约测试矩阵）
> 当前候选基线：`57c474b2e22778163a2ffc9daf12f900abf3ac37`（当前 main；D07 复核快照与历史候选仍保留作审计对照）
> 状态：`draft-2026-08-26`，目标契约仍待评审冻结；当前实现与目标差异见 [D07 当前实现索引](28_D07服务接口数据归属与需求追溯.md)
> 范围：CR-01～CR-06；不包含骑手、派单、轨迹、运力和骑手结算

> 当前实现注记（2026-09-02）：本文保留“三服务”目标契约历史名称，不是当前运行数量的来源。当前为 identity、merchant、order、assistant 四个独立服务，源码业务 mapping 为 13/30/19 + 1 个评价投影/1 个 assistant，共 64 个；库存结果、Outbox/Inbox、Saga 和过期预占状态以 [`D07`](28_D07服务接口数据归属与需求追溯.md)、[`架构矩阵`](MICROSERVICE_FINAL_ARCHITECTURE_MATRIX.md) 和 `docs/contracts/` 当前契约为准。

## 1. 本文如何使用

本文是 `docs/15_单体后端审计与三服务拆分草案.md` 的目标契约细化，不把目标数据库、JWT、消息基础设施或外部目标接口误报为当前已全量落地。外部 `/api/v1` 契约保持兼容；当前代码已经有四个独立服务和 64 个内部业务 mapping，但 BFF 仍保留兼容路由，部分能力仍回落到 `DemoStore`。源码、测试和 [`28_D07服务接口数据归属与需求追溯.md`](./28_D07服务接口数据归属与需求追溯.md) 是当前实现事实，本文件和 `docs/contracts/` 是目标契约候选。

### D07 当前实现摘要

| 口径 | 当前事实 |
|---|---|
| 当前服务边界 | identity 8081（13）、merchant 8082（30）、order 8083（19 + 1 评价投影）、assistant 8084（1），共 64 个业务 mapping；本文件其余目标契约表仍是历史草案 |
| 对外入口 | backend 继续承接 `/api/v1/**`；前端不直接调用 `/internal/v1/**` |
| 数据库 | identity、merchant、order 分别使用三个 service DB；assistant 无业务 DB；legacy `life_assistant` 仅用于兼容/迁移/回滚 |
| 未完成能力 | 深层故障注入、人工恢复、更大样本性能和多节点/metrics-server 故障联合实验仍需补；merchant-service HPA 的单节点 1→2→3→1 现场证据已具备 |
| 详细追溯 | 见 D07 文档的接口表、数据归属表、REQ→UC→代码→测试表和单体差异表 |
...
| DevOps | 分别发布四个服务；仅网关暴露业务 API，内部 API 只在集群网络可达；按第 10 节配置探针、超时与追踪 |
...
| `inventory.reserve.requested` | order | merchant | `orderId,expiresAt,items[]` | 幂等预占库存 |
| `inventory.confirm.requested` | order | merchant | `orderId` | 仅在支付成功后的 RESERVED 预占上确认 |
| `inventory.release.requested` | order | merchant | `orderId,failureStatus` | 补偿或释放未支付预占 |
| `inventory.result.reserved/confirmed` | merchant | order | `orderId,reservationStatus,sourceEventId` | 推进预占/确认 Saga |
| `inventory.result.released` | merchant | order | `orderId,reservationStatus=RELEASED,outcome=RELEASED` | 仅此结果允许 Saga 进入 `RELEASED` |
| `inventory.result.check_required` | merchant | order | `orderId,reservationStatus=CHECK_REQUIRED,outcome=CHECK_REQUIRED,error` | 进入人工核对终态，不得自动确认或释放 |
| `inventory.result.release_failed` | merchant | order | `orderId,reservationStatus=FAILED,outcome=RELEASE_FAILED,error` | 进入 `RELEASE_FAILED`，保留重试/告警依据 |

事件交付语义为 at-least-once：允许重复和乱序，不允许消费者依赖“只会收到一次”。消费者先写 `inbox_event` 去重，再在本地事务中更新投影；order 只接受合法的前置 Saga 状态，晚到结果不得覆盖 `RELEASED`、`CHECK_REQUIRED` 或 `RELEASE_FAILED`。无法识别的更高 major version 进入死信队列并告警。
