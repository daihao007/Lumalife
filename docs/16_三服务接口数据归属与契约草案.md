...
> 当前候选基线：`origin/main@f6fa4a3`（已含 `ROLE_USER`、`/auth/me` 和 `DELIVERING → COMPLETED` 修复；当前分支实测后端 57/57，前端 17/17）
...
本文是 `docs/15_单体后端审计与三服务拆分草案.md` 的契约细化，不改变现有单体代码，也不宣称数据库或消息基础设施已经落地。源码仍是当前行为事实；本文件和 `docs/contracts/` 是三服务迁移的冻结候选。
...
|---|---|---|---|---|---|---|
| ID-01 | POST | `/api/v1/auth/login` | Public | `phone,password` | `AuthResult` | 400 `VALIDATION_FAILED`；401 `INVALID_CREDENTIALS` |
| ID-02 | POST | `/api/v1/auth/register` | Public | `phone,password,nickname`; `role` 不再接受提权 | `AuthResult` | 400；409 `PHONE_ALREADY_REGISTERED` |
| ID-03 | POST | `/api/v1/auth/register/merchant` | Public/受控 | `phone,password,nickname,invitationCode?` | `AuthResult` | 400；403 `MERCHANT_REGISTRATION_DENIED`；409 |
...
`UserView` 绝不包含 `password` 或密码哈希。`AuthResult` 包含 `token`、`user`，目标态可新增 `refreshToken` 和 `expiresIn`。
...
|---|---|---|---|---|
| identity | `user_account` | identity | `phone` 唯一；role 白名单；password_hash 非空 | userId、role、merchantId、显示名投影 |
...
| order | `payment_record` | order | `(user_id,order_id,client_request_id)` 唯一 | 无 |
...
| INT-MER-02 | order → merchant | `POST /internal/v1/inventory/reservations` | 1 s | 同 key 最多 2 次 | 409 库存不足；超时后先按 orderId 查询/重试，不能盲目重建 |
| INT-MER-03 | order → merchant | `POST /internal/v1/inventory/reservations/{orderId}:release` | 1 s | 后台指数退避 | 幂等；支付成功后不得释放 |
...
| `MerchantAccountBound.v1` | identity | merchant | `userId,merchantId,nickname,boundAt` | 幂等建立账户绑定 |
...
| CT-ORD-03 | order / frontend | 支付幂等并发 | 唯一键竞争只有一个 SUCCESS；同 key 同结果；库存不重复预占 |
| CT-ORD-04 | order / merchant | 支付本地事务失败 | 触发幂等 release；失败可重试/告警；订单不误置 PAID |
| CT-ORD-05 | order / frontend | 状态机 | 只允许文档定义边；非法跳转 409 且版本不变 |
| CT-ORD-06 | order / frontend | 一单一评 | 仅完成/已收货订单可评；并发第二次评价 409 |
...
| 事件契约 | 8 个事件有版本、公共信封、去重和最小 payload | AsyncAPI 与第 7 节 |
| 前端兼容 | v1 路径和成功外壳保持；已知安全缺陷不冻结 | 第 2、4 节 |
| 测试可执行性 | 23 个编号测试点覆盖 provider、consumer、事件和 E2E | 第 9 节 |
...
- [x] 7 个内部调用定义超时、重试和失败语义；
- [x] 8 个版本化事件及公共信封已形成 AsyncAPI 草案；
- [x] 错误码、稳定 reason、追踪和安全文案已统一；
- [x] 23 个契约测试点和 DevOps 运行契约已列出；
