...
> 当前候选基线：`fd068de1254233ad14bdd1a24e39d3b10f3b688d`（D07 文档核对代码快照；已含三项独立服务、内部 API、远程适配器和 D06 契约测试矩阵）
...
本文是 `docs/15_单体后端审计与三服务拆分草案.md` 的目标契约细化，不把目标数据库、JWT、消息基础设施或 52 个外部接口误报为当前已全量落地。外部 `/api/v1` 契约保持兼容；当前代码已经有三个独立服务和 38 个内部业务接口，但 BFF 仍保留兼容路由，部分能力仍回落到 `DemoStore`。源码、测试和 [`28_D07服务接口数据归属与需求追溯.md`](./28_D07服务接口数据归属与需求追溯.md) 是当前实现事实，本文件和 `docs/contracts/` 是目标契约候选。

### D07 当前实现摘要

| 口径 | 当前事实 |
|---|---|
| 独立服务 | identity 8081、merchant 8082、order 8083，内部业务接口分别 9/12/17，共 38 个 |
| 对外入口 | backend 继续承接 `/api/v1/**`；前端不直接调用 `/internal/v1/**` |
| 数据库 | merchant/order 使用共享 MySQL `life_assistant` 中的逻辑服务表；identity 使用 JSON 状态文件，不是独立 Schema |
| 未完成能力 | merchant 团购持久化、库存预占/释放、order 多商品明细、Outbox/Inbox/事件总线、客服/AI/指标服务化仍未完成 |
