...
> 当前实现注记（2026-09-02）：本文保留“三服务”目标契约历史名称，不是当前运行数量的来源。当前为 identity、merchant、order、assistant 四个独立服务，源码业务 mapping 为 13/30/20 + 1 个评价投影/1 个 assistant，共 65 个；库存结果、Outbox/Inbox、Saga 和过期预占状态以 [`project-facts.md`](project-facts.md) 和 [`架构矩阵`](MICROSERVICE_FINAL_ARCHITECTURE_MATRIX.md) 为准。

## 1. 本文如何使用

本文是 `docs/15_单体后端审计与三服务拆分草案.md` 的目标契约细化，不把目标数据库、JWT、消息基础设施或外部目标接口误报为当前已全量落地。外部 `/api/v1` 契约保持兼容；当前代码已经有四个独立服务和 65 个内部业务 mapping，但 BFF 仍保留兼容路由。源码、测试和 [`project-facts.md`](project-facts.md) 是当前实现事实，本文件和 `docs/contracts/` 是目标契约候选。

### D07 当前实现摘要

| 口径 | 当前事实 |
|---|---|
| 当前服务边界 | identity 8081（13）、merchant 8082（30）、order 8083（20 + 1 评价投影）、assistant 8084（1），共 65 个业务 mapping；本文件其余目标契约表仍是历史草案 |
