...
`12_软件需求规格说明书.md`、`13_概要设计说明书.md`、`14_详细设计说明书.md` 及其三份同名 `10组-*.pdf` 已更新为当前微服务验收版。其余 `00~11`、四份非核心正式 PDF 以及 `diagrams/d03|d04` 仍主要描述单体/兼容层或历史阶段；相关类仍可用于 `monolith-start` 基线与回滚，但不能据此说明 `prod,remote` 的当前服务调用关系。

## PDF 状态

三份核心正式 PDF（需求、概要、详细设计）已于 2026-09-03 从当前 Markdown 重新生成，并完成 8/7/8 页逐页视觉检查。其余四份 PDF 仍为历史导出，测试、部署和用户文档尚需按最终口径重导出；因此整体 PDF 交付状态为 **⚠️ 部分完成**，不得把未校准的历史 PDF 当作当前版本。
