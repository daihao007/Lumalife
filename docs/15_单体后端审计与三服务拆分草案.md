...
> 当前候选基线：`origin/main@f6fa4a3`（2026-08-26），已包含 `ROLE_USER` 路径限制、`/auth/me` 认证修复和 `DELIVERING → COMPLETED` 页面入口；rebase 后实测 57/57（评审回复中的 46/46 属于合并前候选快照）
...
| 测试 | 旧标签：30 个 `DemoStoreTest` + 10 个 MockMvc 安全集成测试；当前候选：36 个 `DemoStoreTest` + 21 个 `ApiSecurityIntegrationTest` | 旧标签 40 个；评审快照 46 个；当前候选 57 个，均为单进程测试，仍需契约、数据库、并发与故障测试 |
...
- 同时记录旧标签 40 个、评审快照 46 个和当前候选 57 个测试结果；新增 `/auth/me` 匿名、角色越权和支付并发测试不得回写旧标签数字；
