...
| `MerchantAdminService` | 商家资料、商品/套餐维护 | `MerchantServicePort` | 商家商品服务门面；订单履约已移至 `OrderAdminService` |
| `OrderAdminService` | 商家订单、评价、履约状态、券码核销 | `OrderServicePort` | 订单服务的商家侧门面 |
...
| `order` | `payment_record` | order | `(user_id,client_request_id)` 唯一并记录 order_id；状态含 `PROCESSING/SUCCESS/FAILED` |
...
| order -> merchant | `GET /internal/v1/inventory/reservations/{orderId}` | 预占 POST 超时后查询确定结果 | 返回 reservation 生命周期状态；404 后才允许同 key/同载荷重放 |
...
| `MerchantRegistrationRequested.v1` | identity | merchant | 以 registrationId 幂等创建商家事实 |
| `MerchantProvisioned.v1` | merchant | identity | 商家建档成功后激活身份账户 |
| `MerchantProvisioningFailed.v1` | merchant | identity | 终态失败、禁止签发 token 并进入补偿 |
...
1. order 服务以 `(userId, clientRequestId)` 唯一索引创建记录了 orderId 的 `PROCESSING` 支付记录；同订单重复请求返回已有结果或当前处理中状态，同 key 指向不同订单返回 409；
