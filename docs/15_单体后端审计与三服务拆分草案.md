...
| 测试 | 30 个 `DemoStoreTest` + 10 个 MockMvc 安全集成测试 | 当前 40 个测试全为单进程基线；需新增契约、数据库、并发与故障测试 |
...
| POST | `/api/v1/auth/register` | Public | 注册普通用户；禁止由请求提升平台角色 |
| POST | `/api/v1/auth/register/merchant` | Public（现状） | 商家入驻账户；目标态应增加审核/受控入口 |
| GET | `/api/v1/auth/me` | 匹配器误设为 Public | 当前身份 |
| POST | `/api/v1/user/profile` | Authenticated | 更新昵称和头像 |
| GET | `/api/v1/user/addresses` | Authenticated | 地址列表 |
| POST | `/api/v1/user/addresses` | Authenticated | 新增或编辑地址 |
| POST | `/api/v1/user/addresses/{id}/default` | Authenticated | 设置默认地址 |
| POST | `/api/v1/user/addresses/{id}/delete` | Authenticated | 删除地址 |
...
| GET | `/api/v1/user/favorites` | Authenticated | 收藏商家列表 |
| POST | `/api/v1/user/favorites` | Authenticated | 收藏商家 |
| POST | `/api/v1/user/favorites/{merchantId}/delete` | Authenticated | 取消收藏 |
| GET | `/api/v1/conversations` | Authenticated | 用户会话摘要 |
| GET | `/api/v1/conversations/{merchantId}` | Authenticated | 用户侧会话消息 |
| POST | `/api/v1/conversations/{merchantId}/messages` | Authenticated | 用户发消息并触发 AI 回复 |
...
| Method | Endpoint | 当前访问要求 | 目标职责 |
|---|---|---|---|
| GET | `/api/v1/cart` | Authenticated | 原始购物车项 |
| GET | `/api/v1/cart/detail` | Authenticated | 带商品快照的购物车明细 |
| POST | `/api/v1/cart/items` | Authenticated | 加购 |
| POST | `/api/v1/cart/items/{productId}` | Authenticated | 修改数量 |
| POST | `/api/v1/cart/items/{productId}/delete` | Authenticated | 删除购物车项 |
| POST | `/api/v1/cart/clear` | Authenticated | 清空购物车 |
| POST | `/api/v1/orders/delivery` | Authenticated | 按商家拆分创建外卖订单 |
| POST | `/api/v1/orders/group-buy` | Authenticated | 创建团购订单 |
| POST | `/api/v1/payments` | Authenticated | 幂等模拟支付与库存预占确认 |
| POST | `/api/v1/orders/{id}/cancel` | Authenticated | 取消待支付订单 |
| POST | `/api/v1/orders/{id}/receive` | Authenticated | 用户确认收货 |
| GET | `/api/v1/orders` | Authenticated | 用户订单列表 |
| POST | `/api/v1/reviews` | Authenticated | 完成订单评价 |
...
| order -> merchant | `POST /internal/v1/inventory/reservations/{orderId}:confirm` | 支付成功确认扣减 | 幂等；失败进入可重试状态 |
...
| `OrderPaid.v1` | order | merchant | 销量/推荐投影、库存确认 |
...
1. `payment_record` 先用 `(userId, orderId, clientRequestId)` 唯一索引保证幂等；
2. order 服务请求 merchant 服务按 `orderId` 预占库存；
3. 预占成功后，order 服务本地事务写支付结果、订单 `PAID`、团购券和 Outbox；
4. merchant 服务消费/接收确认并完成库存扣减；
5. 订单事务失败或支付超时则幂等释放预占；后台任务扫描并修复悬挂 Saga。
...
| P1 | `/auth/**` 全部 Public，`/auth/me` 匿名可能进入空 Principal | 应为 401 的请求可能变成 500 | 仅 login/register Public；补匿名 `/auth/me` 契约测试 |
| P1 | 普通认证路径未限定 USER，商家/管理员可尝试购物车和下单 | 角色边界泄漏 | 网关与服务双层授权；业务命令校验角色和资源归属 |
| P1 | 公共 `/auth/register/merchant` 可直接创建商家管理员 | 未审核入驻和权限扩张 | 受控邀请/审核；至少限流、审计和验证码 |
...
- 固化 40 个现有测试结果，并补 `/auth/me` 匿名、角色越权、支付并发测试；
...
3. 使用何种事件基础设施；即使暂用数据库 Outbox 轮询，也必须先冻结事件 Schema；
