...
assert_query 'versioned migrations' '14' "SELECT COUNT(*) FROM schema_migration WHERE version IN ('V001', 'V002', 'V003', 'V004', 'V005', 'V006', 'V007', 'V008', 'V009', 'V010', 'V011', 'V012', 'V013', 'V014')"
assert_query 'domain tables' '31' "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('category','merchant','user_account','user_address','auth_session','product','group_deal','cart_item','order_main','order_item','order_status_timeline','payment_record','coupon','review','merchant_favorite','chat_message','operation_log','business_state','merchant_catalog','order_record','service_cart_item','service_payment','service_coupon','service_review','service_order_event','service_order_line','service_outbox_event','inventory_reservation','inventory_reservation_item','merchant_inbox_event','schema_migration')"
...
assert_query 'merchant service tables' '10' "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = '${MYSQL_MERCHANT_DATABASE}' AND table_name IN ('schema_migration','category','merchant','merchant_catalog','group_deal','merchant_favorite','chat_message','inventory_reservation','inventory_reservation_item','merchant_inbox_event')"
...
assert_query 'identity service migrations' '14' "SELECT COUNT(*) FROM ${MYSQL_IDENTITY_DATABASE}.schema_migration WHERE version IN ('V001', 'V002', 'V003', 'V004', 'V005', 'V006', 'V007', 'V008', 'V009', 'V010', 'V011', 'V012', 'V013', 'V014')"
assert_query 'merchant service migrations' '14' "SELECT COUNT(*) FROM ${MYSQL_MERCHANT_DATABASE}.schema_migration WHERE version IN ('V001', 'V002', 'V003', 'V004', 'V005', 'V006', 'V007', 'V008', 'V009', 'V010', 'V011', 'V012', 'V013', 'V014')"
assert_query 'order service migrations' '14' "SELECT COUNT(*) FROM ${MYSQL_ORDER_DATABASE}.schema_migration WHERE version IN ('V001', 'V002', 'V003', 'V004', 'V005', 'V006', 'V007', 'V008', 'V009', 'V010', 'V011', 'V012', 'V013', 'V014')"
