...
assert_query 'versioned migrations' '3' "SELECT COUNT(*) FROM schema_migration WHERE version IN ('V001', 'V002', 'V003')"
assert_query 'domain tables' '19' "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('category','merchant','user_account','user_address','auth_session','product','group_deal','cart_item','order_main','order_item','order_status_timeline','payment_record','coupon','review','merchant_favorite','chat_message','operation_log','business_state','schema_migration')"
