...
export default function Detail({ detail, addCart, openCart, buyDeal, backHome, contactMerchant, setMessage }: any) {
  ...
  return <div className="detail" data-testid="merchant-detail-page">
    <button data-testid="detail-back" className="back-button" onClick={backHome}>返回发现</button>
    ...
    <div className="split">
      <section><h3>菜品</h3>{detail.products.map((p: Product) => <div className="line" data-testid={`detail-product-${p.id}`} key={p.id}><span><b data-testid={`detail-product-name-${p.id}`}>{p.name}</b><small>{p.description}</small></span><strong>{money(p.priceCent)}</strong><button data-testid={`add-cart-${p.id}`} disabled={pendingAction !== null} onClick={() => runAction(`cart-${p.id}`, () => addCart(p.id))}>{pendingAction === `cart-${p.id}` ? "加购中…" : "加购"}</button></div>)}<div className="actions"><button data-testid="detail-open-cart" className="primary" disabled={pendingAction !== null} onClick={openCart}>去购物车结算</button><button data-testid="contact-merchant" disabled={pendingAction !== null} onClick={() => contactMerchant(detail.merchant.id)}>联系店家客服</button></div></section>
